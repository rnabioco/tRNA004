...
plot_modification_data <- function(mapped, shortname) {
  ...
  # Retrieve the corresponding mod_nt
  ...
  plot_mismatch <- ggplot(filtered_data, aes(x = mod_context, y = MismatchFreq)) +
    ...
    labs(title = paste("Mismatch frequencies in different nucleotide contexts,", shortname, "(", mod_nt_value, ")"),
         ...
         y = "Mismatch frequency") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, margin = margin(2, 0, 2, 0), face = "bold", color = "black", size = 10),
...
plot_modification_data(mapped, "t6A")
```
