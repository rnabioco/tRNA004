...
yields$SpeciesSample <- factor(paste(yields$Species, yields$Sample), 
                               levels = c("Yeast pus4del", "Yeast S288C", 
                                          "Drosophila S2", "Human HS27", 
                                          "Tetrahymena thermophila", "Zebrafish D_rerio"))

```
And then we plot.
```{r}
yields %>%
  ggplot(aes(x = SpeciesSample, y = RPPPM, fill = Chemistry)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  labs(x = "Species and Sample", y = "RPPPM", fill = "Chemistry") +
  theme_cowplot() +
