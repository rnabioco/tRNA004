...
```
We want to compare alignment rates and other key stats from our tRNA sequencing runs across multiple organisms, looking at various basecalling models for both chemistries. I have two metafiles from parsing lines in the fastq files, and from running samtools stats on the bam files. Can pull both of these in here now.
```{r}
stats <- read_tsv(here("samtools_stats_summary.txt")) %>%
  mutate(Filename = gsub("\\.bwa\\.bam$", "", Filename))

counts <- read_tsv(here("fastq_read_counts.txt")) %>%
...
all %>%
  ...
  scale_fill_OkabeIto(order = c(1,6,2,5,3)) #order = c(3,7)
```
