...
mapped %>%
  ...
  ggplot(aes(x = shortname, y = BCErrorFreq)) +
    ...
    geom_hline(yintercept = 0.2363636, linetype = "dashed", color = "#F0E442") + # unmodified Us
    geom_hline(yintercept = 0.1524008, linetype = "dashed", color = "#D55E00") + # unmodified Gs
...
mapped %>%
  ...
  ggplot(aes(x = shortname, y = BCErrorFreq)) +
    ...
    geom_hline(yintercept = 0.3333333, linetype = "dashed", color = "#F0E442") + # unmodified Us
    geom_hline(yintercept = 0.2000000, linetype = "dashed", color = "#D55E00") + # unmodified Gs
#    geom_jitter(cex = 0.2, alpha = 0.4, size = 0.5, width = 0.25) +  # Jitter plot
    geom_boxplot(width = 0.2, alpha = 0.75, fill = "darkgray", outlier.size = 0.5) +
    theme_bw() +
    labs(title = "Basecalling error frequency across unique sequence contexts on tRNA isodecoders",
...
mapped %>%
  ...
  filter(Position > 0 & Position < 98) %>%
  ggplot(aes(x = shortname, y = BCErrorFreq, fill = chemistry)) +
    geom_boxplot(position = position_dodge(width = 0.75),  width = 0.5, outlier.size = .5) +
