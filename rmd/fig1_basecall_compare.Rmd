...
```{r}
# Preprocess the Species column to desired format
fl <- fl %>%
  mutate(SpeciesFormatted = case_when(
    Species == "D_melanogaster" ~ "italic('D. melanogaster')",
    Species == "E_coli" ~ "italic('E. coli')",
    Species == "H_sapiens" ~ "italic('H. sapiens')",
    Species == "S_cerevisiae" ~ "italic('S. cerevisiae')",
    Species == "T_thermophila" ~ "italic('T. thermophila')",
    Species == "D_rerio" ~ "italic('D. rerio')",
    TRUE ~ NA_character_
  ))

# Filter and plot
fl %>%
  filter(!Sample %in% c("Trub1doxH295R", "Trub1untrH295R", "Pus4yeast", "Grandeyeast", "Petityeast", "Pus1yeast", "Pus2yeast")) %>%
  filter(!is.na(Species)) %>%
  ggplot(aes(x = SpeciesFormatted, y = PercentAligned, fill = interaction(BCModel, Chemistry, Version))) +
  geom_bar(stat = "identity", position = position_dodge()) +
  ...
  scale_x_discrete(labels = function(x) parse(text = x)) + # Parse text for axis labels
  scale_fill_OkabeIto(order = c(6, 1, 4, 3, 2, 5, 7, 8), use_black = TRUE, na.translate = FALSE)
```
...
```
Write out info for table S2.
```{r}
all %>%
  filter(ReadsMapped != "NA") %>%
  select(-Species) %>%
  write.csv(here("metadata/ts2start.csv"))
...
test <- test %>%
  mutate(SpeciesFormatted = case_when(
    Species == "D_melanogaster" ~ "D. melanogaster",
    Species == "E_coli" ~ "E. coli",
    Species == "H_sapiens" ~ "H. sapiens",
    Species == "S_cerevisiae" ~ "S. cerevisiae",
    Species == "T_thermophila" ~ "T. thermophila",
    Species == "D_rerio" ~ "D. rerio",
    TRUE ~ NA_character_
  ))


ggplot(test, aes(x = SpeciesFormatted, y = PercentAligned, fill = filtering)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, face = "italic"),
        plot.margin = margin(5.5, 5.5, 5.5, 40, "points")) +
