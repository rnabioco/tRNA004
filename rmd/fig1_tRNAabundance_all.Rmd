...
Plotting function & correlation plots
```{r}
plot_species <- function(data, species_name) {
  ...
  x_range <- range(species_data$RNA002_hac, na.rm = TRUE)
  y_range <- range(species_data$RNA004_sup, na.rm = TRUE)
  ...
  plot <- ggplot(species_data, aes(x = RNA002_hac, y = RNA004_sup, color = isoacceptor)) +
    ...
    labs(title = bquote(italic(.(species_name))), x = NULL, y = NULL) +
    theme_cowplot() +
    theme(aspect.ratio = 1,
          plot.margin = margin(5.5, 5.5, 15.5, 5.5, "points"),
          plot.title = element_text(hjust = 0.5, face = "italic"),
          axis.title.x = element_blank(),
          axis.title.y = element_blank()) +
    coord_fixed(ratio = 1) +
    annotate("text", x = x_range[1], y = y_range[2], 
             label = sprintf("ρ: %.2f", cor_coefficient), 
...
# Example usage
all_species <- levels(cpm_comparison_complete$species)
...
combined_plot <- combined_plot +
  ...
  labs(x = "RNA002", y = "RNA004")

suppressWarnings(print(combined_plot))
```


Then use the isodecoder scores data to generate a PCA plot.
```{r}
isodecoder_scores %>%
  ungroup() %>%
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = species, shape = chemistry)) +
  ...
  theme(aspect.ratio = 1, legend.text = element_text(face = "italic"))
